...
- To use mocked hardware (capability of ros2_control), use `use_fake_hardware` argument, like:
  ...
  **NOTE**: Instead of using global launch file for control stack, there are also prepeared launch files for each type of UR robots named. The accept the same arguemtns are the global one and are used by:
  ```
