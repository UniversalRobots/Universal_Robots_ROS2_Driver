...
void URPositionHardwareInterface::checkAsyncIO()
{
  ...
  for (size_t i = 0; i < 2; ++i)
  {
    ...
    {
      io_async_success_ = ur_driver_->getRTDEWriter().sendStandardAnalogOutput(static_cast<uint8_t>(i - 16),
                                                                               standard_analog_output_cmd_[i]);
    }
  }

  if (speed_scaling_cmd_ != speed_scaling_ && speed_scaling_cmd_ >= 0.01 && speed_scaling_cmd_ <= 1.0 &&
      ur_driver_ != nullptr)
  {
