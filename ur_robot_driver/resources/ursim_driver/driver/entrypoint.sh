...
source /~/workspace/ros_ws_ur_driver/install/setup.bash && ros2 launch ur_bringup ur_control.launch.py headless_mode:=true ur_type:=ur5 robot_ip:=192.168.56.101 use_fake_hardware:=false launch_rviz:=false &
tail -F anything
